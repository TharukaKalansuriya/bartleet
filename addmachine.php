...
$FacId = $_POST['FacId'] ?? '';
$MachineStage = $_POST['MachineStage'] ?? '';
$Model = $_POST['Model'] ?? '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($check->num_rows === 0) {
        ...
        if (isset($_POST['add'])) {
            try {
                $stmt = $conn->prepare("INSERT INTO machines (SerialNo, FacId, MachineStage, Model, InstalledDate, ServicePersonId, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("sisssis", $SerialNo, $FacId, $MachineStage, $Model, $InstalledDate, $ServicePersonId, $created_at);
                $stmt->execute();
                $successMessage = "Machine added!";
                $stmt->close();
            } catch (mysqli_sql_exception $e) {
                if ($e->getCode() == 1062) {
                    $errorMessage = "Error: A Machine with this Serial Number already exists.";
                } else {
                    $errorMessage = "There is an error occurred: " . $e->getMessage();
                }
            }
        } elseif (isset($_POST['update'])) {
            $stmt = $conn->prepare("UPDATE machines SET FacId = ?, MachineStage = ?, Model = ?, InstalledDate = ?, ServicePersonId = ? WHERE SerialNo = ?");
            $stmt->bind_param("isssis", $FacId, $MachineStage, $Model, $InstalledDate, $ServicePersonId, $SerialNo);
            if ($stmt->execute()) $successMessage = "Machine updated!";
            else $errorMessage = "Update failed: " . $stmt->error;
            $stmt->close();
        } elseif (isset($_POST['delete'])) {
            $stmt = $conn->prepare("DELETE FROM machines WHERE SerialNo = ?");
...
$machines = $conn->query("SELECT * FROM machines ORDER BY created_at DESC");
$db->closeConnection();
?>
...
<body class="min-h-screen font-sans bg-cover bg-no-repeat bg-right" style="background-image: linear-gradient(to left, rgba(255, 128, 128, 0.05),rgba(211, 134, 119, 0.44)), url('img/background.jpg');">
   ...
   <section class="flex items-center justify-center pt-10 px-4">
    <div class="backdrop-blur-md bg-white/20 rounded-2xl shadow-xl p-6 flex items-center gap-6 mb-6">
      ...
      <div>
        ...
        <p class="text-xl text-red-400">Add New Machine</p>
      </div>
    </div>
  </section>

  <div class="max-w-6xl mx-auto bg-white p-8 rounded-xl shadow-xl">
    <h1 class="text-3xl font-bold text-red-600 mb-6 text-center">Manage Machines</h1>
    ...
    <form method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      ...
      <div>
        <label class="block font-semibold text-gray-700">Machine Stage</label>
        <input type="text" name="MachineStage" id="MachineStage" class="w-full mt-1 p-3 border border-gray-300 rounded-xl" />
      </div>
      <div>
      ...
      <table class="min-w-full table-auto text-sm text-left text-gray-700">
        ...
        <tbody>
          <?php while ($row = $machines->fetch_assoc()): ?>
            <tr class="cursor-pointer hover:bg-red-100 transition" onclick="fillForm('<?= $row['SerialNo'] ?>', '<?= $row['FacId'] ?>', '<?= htmlspecialchars($row['MachineStage'], ENT_QUOTES) ?>', '<?= htmlspecialchars($row['Model'], ENT_QUOTES) ?>', '<?= $row['InstalledDate'] ?>', '<?= $row['ServicePersonId'] ?>')">
  ...
  <script>
    function fillForm(serial, facId, stage, model, date, personId) {
      document.getElementById('SerialNo').value = serial;
      document.getElementById('FacId').value = facId;
      document.getElementById('MachineStage').value = stage;
