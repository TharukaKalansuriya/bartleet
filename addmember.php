...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add'])) {
        try {
            $stmt = $conn->prepare("INSERT INTO Members (ServicePersonId, NAME, Location) VALUES (?, ?, ?)");
            $stmt->bind_param("sss", $ServicePersonId, $Name, $Location);
            $stmt->execute();
        ...
        $stmt = $conn->prepare("UPDATE Members SET NAME = ?, Location = ? WHERE ServicePersonId = ?");
